...
if(testeTPP12>0){
  ...
  myDFFF <- myDF %>% select(local,barcode,genotipo,rep,trait,Nota)
  myDF <- na.omit(myDFFF)
}else{
  myDF <- allData_filter %>% filter(trait %in% filterTrait$codigo[1], stg %in% myStag) %>%
  select(local,barcode,genotipo,rep,trait,Nota)
  myDF <- na.omit(myDF)
}
...
# Filtrar por local
entradasLoc <- entradasLoc %>% filter(Entradas>30, Entradas<500)
