...
checksCS <- readxl::read_xlsx("../data/renataMilho27062022.xlsx", sheet="Checks")
checksCS$Esperado <- as.numeric(checksCS$Esperado)
sinN <- readxl::read_xlsx("../data/renataMilho27062022.xlsx", sheet="Sinonimos")
# sinN <- readxl::read_xlsx("../data/original_suporteMilho.xlsx", sheet="Sinonimos")
...
tryCatch(tidyr::gather(checksCS,TPP,valor,6:9),
 error = function(e)
 errorMessage <<- TRUE)
...
if(errorMessage==T){
  ...
  checksCS <- tidyr::gather(checksCS,TPP,valor,6:9)
}else{
  checksCS <- tidyr::gather(checksCS,TPP,valor,6:9)
}
...
myStag <- c(5) # Aqui seleciona o estagio


# myStag <- 6
allData_filter <- allData %>% filter(TPP %in% myTPP)
allData_filter$stg <- gsub(8,6,allData_filter$stg) # transformando estagio 8 em 6
allData_filter$stg <- gsub("L",4,allData_filter$stg)

#Analisar yiedl junto?
duasT <- "no"

#limites
limiteAlta <- -0.5 #Desvio a partir de 0 passa a ser alta
...
makePie(dataPointsTable, paste0("4,5,6"))

allData <- sinoFun(allData,sinN)

# writexl::write_xlsx(allData, "../output/rawData.xlsx" )
...
myDF <- allData_filter %>% filter(trait %in% filterTrait$codigo[1], stg %in% myStag) %>%
  select(BU,local,barcode,stg,genotipo,rep,trait,Nota,TPP)
...
checksF$Genotipo <- gsub(" ", "", checksF$Genotipo, fixed = TRUE)
checksF$Genotipo <- toupper(checksF$Genotipo)
colnames(checksF) <- c("genotipo", "Check", "Trait", "Esperado", "Tipo")

checksF<-sinoFun(checksF,sinN)
# Corrigindo nome das colunas
checksF <- checksF[,-7]
colnames(checksF)[6] <- "TPP"
checksF <- checksF %>% filter(TPP %in% myTPP)
...
if(is.na(myHB)==F){
  ...
  tableCount$TPP <- myTPP
  DT::datatable(tableCount, options=list(search=list(regex=TRUE, search = '')))
}else{
  for(i in 1:length(myStag)){
    tableCount <- count_fun(tableTraits,allData_filter,"local")
    makePie(tableCount, myStag[i])
    tableCount$stage <- myStag[i]
    tableCount$TPP <- myTPP
...
writexl::write_xlsx(tableCount, paste0("../output/",myTPP,"_count-",Sys.Date(),".xlsx"))
...
# procurar por um nome especifico
# match("TRUE",str_detect(myDF$genotipo,"30F53VYH"))
...
colnames(checksFora)[6] <- "TPP"

if(length(checksFora) == 7){
  checksFora <- checksFora[,-7]
}
...
genLoc <- genLoc %>% select(genotipo,local,TPP,blueN,classe,stage)

writexl::write_xlsx(genLoc,paste0("../output/",myTPP,"_genoLoc-",Sys.Date(),".xlsx"))
...
for(i in 1:){
  i=13
  myGraph <- myDF6 %>% filter(BU %in% finalTabelaBU$Local[i])
  # myGraph <- myDF6 %>% filter(BU %in% finalTabela2$Local[i])
  # p1 <- makeG0(myGraph,"genotipo","Nota")
  # p2 <- makeH0(myGraph,"Nota")
  p3 <- makeG9(myGraph,c("MS","MT"),"TPP11")
  p4 <- makeG9(myGraph,c("S","T"),"TPP11")
...
modeloBU <- joint_blup(altaPPBU, "no") # yes = local efeito fixo, no = local nao entra como efeito
modeloBU$type <- "BU"
...
nomeConjuntaGenotipos <- paste0("../output/conjuntaGenotiposCS_",Sys.Date(),".xlsx")
writexl::write_xlsx(conjuntaGenotipos,nomeConjuntaGenotipos)
...
allData_filter <- sinoFun(allData_filter,sinN)
