...
selTrait <- c(tableTraits$codigo[1],"yieldGSM")
filterTrait <- tableTraits %>% filter(codigo %in% selTrait)
...
removeGenotipo <-c("MG580PWU", "NK505VIP3", "K9609VIP3")
...
fileChecks <- myDF %>% filter(genotipo %in% checksF$genotipo) %>% dplyr::select(local,BU,genotipo)
...
teste <- myDF %>% tidyr::spread(trait, Nota)
