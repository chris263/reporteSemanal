...
if(testeTPP12>0){
  ...
  myDFFF <- myDF %>% select(BU,local,barcode,genotipo,rep,trait,Nota)
  myDF <- na.omit(myDFFF)
}else{
  myDF <- allData_filter %>% filter(trait %in% filterTrait$codigo[1], stg %in% myStag) %>%
  select(BU,local,barcode,genotipo,rep,trait,Nota)
  myDF <- na.omit(myDF)
}
...
entradasLoc <- as.data.frame(tapply(myDF$genotipo, myDF$BU,length))
...
# Filtrar por local
entradasLoc <- entradasLoc %>% filter(Entradas>30, Entradas<5000)
...
BLUEs <- local_blup(dfLocal,duasT, myDF) # arquivo selectionIndex.R
...
altaPP <- myDF %>% filter(BU %in% locAlta$Local)
