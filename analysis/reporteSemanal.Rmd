...
rawPheno7 <- read.csv("../data/TPP12_10062022.csv", header=T, sep=",")
...
myDF7 <- data.frame(local=as.character(rawPheno7$PSS.EXT.EXTID),genotipo=as.character(rawPheno7$ABBRC),
                   rep=as.numeric(rawPheno7$PSS.REPNO),
                   COSTR=as.numeric(rawPheno7$PSS.COSTR),
                   HELMR=as.numeric(rawPheno7$PSS.HELMR),
                   GRLSR = NA, # GRLSR=as.numeric(rawPheno7$PSS.GRLSR),
                   LFSPR=as.numeric(rawPheno7$PSS.LFSPR),
                   TPP="TPP12",
                   range=as.numeric(rawPheno7$PSS.MAXRNG), row=as.numeric(rawPheno7$PSS.MAXROW))
...
allData_hb <- allData %>% mutate(criteria = str_detect(allData$genotipo,"6101")) %>% filter(criteria == "TRUE") %>% select(!criteria)
