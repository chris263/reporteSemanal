...
checksCS <- readxl::read_xlsx("../data/renataMilho21062022.xlsx", sheet="Checks")
...
myStag <- c(6) # Aqui seleciona o estagio
...
myDF6 <- myDF %>% filter(stg == 6)
match("TRUE", str_detect(finalTabela2$Local,"BC08"))

for(i in 1:2){
  i=8
  myGraph <- myDF6 %>% filter(BU %in% finalTabela2$Local[i]) %>% filter(genotipo %in% checksF$genotipo)
