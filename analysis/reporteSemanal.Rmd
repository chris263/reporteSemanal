...
myDF <- allData_filter %>% filter(trait %in% filterTrait$codigo[1], stg %in% myStag) %>%
  select(BU,local,barcode,stg,genotipo,rep,trait,Nota)
...
BLUEs <- data.frame(genotipo=NA,local=NA,blueN=NA,stage=NA, type=NA)
classificaF <- data.frame(Local=NA,classe=NA,Probabilidade=NA,Controles=NA,Desvio=NA,stage=NA, type=NA)
resumoLocF <- data.frame(local=NA,H2N=NA,H2Y=NA,Acuracia=NA,stage=NA, type=NA)
