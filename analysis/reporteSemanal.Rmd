...
nomeSalvar <- paste0("../output/blupDoencas",Sys.Date(),".xlsx")
nomeSalvarBU <- paste0("../output/BU_Doencas",Sys.Date(),".xlsx")
...
checksCS <- readxl::read_xlsx("../data/original_suporteMilho.xlsx", sheet="Checks")
...
# match("TRUE",str_detect(myDF$genotipo,"30F53"))
...
novaTabela_Trial <- left_join(classifica_BU,resumoBU,by=c("Local"="local")) 
...
finalTabela2 <- finalTabela %>% dplyr::select(Local,TPP,classe,stage,Controles,Desvio,Acuracia) %>% arrange(desc(Desvio))# Pode Adicionar Acuracia 
...
for(i in 1:length(unique(finalTabela2$Local))){
  # i=1
  myGraph <- myDF %>% filter(BU %in% finalTabela2$Local[i]) %>% filter(genotipo %in% checksF$genotipo)
...
for(j in 1:length(listTraits)){
  ...
  filterMean <- allData_filter %>% filter(trait %in% listTraits[j]) %>%
    select(TPP,stg,local,BU,genotipo,barcode,rep,Nota)
  ...
  locMean <- data.frame(tapply(filterMean$Nota,list(filterMean$local,filterMean$stg), mean, na.rm=T))
  ...
  for(i in 1:length(locCols)){
    ...
    checkModel = 0
    if(length(unique(filterMean2$local))>1 & length(unique(filterMean2$rep))>1){
      # modelo2 <- aov(Nota~genotipo+local+rep, data = filterMean2)
      modeloLocalM <- lmer(Nota~local+rep+(1|genotipo), data=filterMean2)
      checkModel = 1
    }else if(length(unique(filterMean2$local))>1 & length(unique(filterMean2$rep))==1){
      # modelo2 <- aov(Nota~genotipo+local, data = filterMean2)
      modeloLocalM <- lmer(Nota~local+(1|genotipo), data=filterMean2)
      checkModel = 1
    }else if(length(unique(filterMean2$local))==1 & length(unique(filterMean2$rep))>1){
